...
- (void)viewDidLoad
{
    ...
    // 告诉singleTapRecognizer，它应该在doubleTapRecognizer断定当前用户输入不是doubleTapRecognizer想要的手势时触发自己操作。
    ...
    // 第一次轻击之后，如果立即发生了第二次轻击，doubleTapRecognizer会完全认为这是我需要的手势，并触发自己的操作。在这是，singleTapRecognizer将意识到所发生的事情，并放弃该手势。另一方面，如果经过了特定时间(系统规定的两次轻击之间最大的时间长度)，doubleTapRecognizer将放弃，singleTapRecognizer看到doubleTapRecognizer失败，最终将触发自己的操作。
